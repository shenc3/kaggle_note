...
def get_label(end_date, n_day):
    ...
    for i in [3, 2, 1, -1]:
        ...
        date_info_temp.rename(
            columns={'holiday_flg': 'ahead_holiday_{}'.format(i),
...
def get_store_visitor_feat(label, key, n_day):
    '''
        提取start_date ~ key[0](end_date)的data数据生成data_temp,
        然后在对应时间区间上的label数据上匹配**整体**聚合数据
    '''
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    result = data_temp.groupby(
        ['store_id'], as_index=False)['visitors'].agg(
...
def get_store_exp_visitor_feat(label, key, n_day):
    '''
        提取start_date ~ key[0](end_date)的data数据生成data_temp,
        然后在对应时间区间上的label数据上匹配加权到店数据
    '''
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    # 将visit_date转化为和key[0]（end_date）的时间差
    data_temp['visit_date'] = data_temp['visit_date'].apply(lambda x: diff_of_days(key[0], x))
    # 利用和结束时间的时间差计算权重：0.986 ** interval
    ...
    # 时间区间内到店人数的加权平均
    result['store_exp_mean{}'.format(n_day)] = result['store_exp_mean{}'.format(n_day)]/\
                                               result['store_exp_weight_sum{}'.format(n_day)]
    result = left_merge(label, result, on=['store_id']).fillna(0)
    return result
...
def get_store_week_feat(label, key, n_day):
    '''
        提取start_date ~ key[0](end_date)的data数据生成data_temp,
        然后在对应时间区间上的label数据上匹配**按星期的**聚合数据
    '''
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
...
def get_store_week_diff_feat(label, key, n_day):
    '''
        求每个商家在时间区间内的一阶差分，然后求一阶差分的mean, std, max, min
        然后将差分的聚合数据匹配到时间区间内的label数据中
    '''
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    ...
    result = left_merge(label, result[
...
def get_store_all_week_feat(label, key, n_day):
    '''
        提取start_date ~ key[0](end_date)的data数据生成data_temp,
        然后计算**按星期的**聚合数据，并将各个星期的聚合数据匹配到label数据上
        air_store_id, dow, *agg_data*, *0agg_data*, *1agg_data*, ..., *6agg_data*
    '''
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
...
def get_store_week_exp_feat(label, key, n_day):
    ...
    for i in [0.9, 0.95, 0.97, 0.98, 0.985, 0.99, 0.999, 0.9999]:
        # 用每个 i和时间间隔(visit_date) 生成权重
        data_temp['weight'] = data_temp['visit_date'].apply(lambda x: i ** x)
        # 计算加权的到店人数visitors1, visitors2和权重和
...
def get_store_holiday_feat(label, key, n_day):
    '''
        提取start_date ~ key[0](end_date)的data数据生成data_temp,
        然后计算时间区间上的<store_id, holiday_flg> 和 <store_id, holiday_flg2>聚合数据
    '''
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
...
predictors = [f for f in test_feat.columns
    if f not in ([
